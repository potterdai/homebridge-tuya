...
class SimpleBlindsAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        var percentState = undefined;
        if (dps.hasOwnProperty(this.dpPercentState)) {
            percentState = dps[this.dpPercentState];
        } else if (dps.hasOwnProperty(this.dpPercentControl)) {
            percentState = dps[this.dpPercentControl];
        }

        console.log("[TuyaAccessory] Acquired percent state: " + percentState);

        console.log("[TuyaAccessory] Update current position: " + this._getCurrentPosition(percentState));
        const characteristicCurrentPosition = service.getCharacteristic(Characteristic.CurrentPosition)
            .updateValue(this._getCurrentPosition(percentState))
            .on('get', this.getCurrentPosition.bind(this));

        console.log("[TuyaAccessory] Update target position: " + this._getTargetPosition(dps[this.dpAction]));
        ...
        this.device.on('change', changes => {
            ...
            if (changes.hasOwnProperty(this.dpAction)) {
                console.log("[TuyaAccessory] Blinds action change to " + changes[this.dpAction]);
            ...
            if (changes.hasOwnProperty(this.dpPercentState)) {
                console.log("[TuyaAccessory] Blinds percent change to " + changes[this.dpPercentState]);
                if (changes[this.dpPercentState]) {
                    ...
                    console.log("[TuyaAccessory] Blinds correct position " + this.assumedPosition);
                }
            }

            if (changes.hasOwnProperty(this.dpPercentControl)) {
                console.log("[TuyaAccessory] Blinds percent change to " + changes[this.dpPercentControl]);
                if (changes[this.dpPercentControl]) {
    ...
    setTargetPosition(value, callback) {
        console.log('[SimpleBlindsAccessory] Blinds asked to move from ' + this.assumedPosition + ' to ' + value);
