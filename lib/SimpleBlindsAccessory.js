...
class SimpleBlindsAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.log("Acquired percent state: " + percentState);

        this.log("Update current position: " + this._getCurrentPosition(percentState));
        ...
        this.device.on('change', changes => {
            ...
            if (changes.hasOwnProperty(this.dpPercentState)) {
                this.log.debug("Blinds percent change to " + changes[this.dpPercentState]);
            ...
            } else if (changes.hasOwnProperty(this.dpPercentControl)) {
                this.log.debug("Blinds percent change to " + changes[this.dpPercentControl]);
...
/*      if (value !== BLINDS_OPEN && value !== BLINDS_CLOSED) {
            this.log.debug("Blinds will stop in " + duration + "ms");
            this.log.debug("Blinds assumed started " + this.changeTime + "; " + (Date.now() - this.changeTime) + 'ms ago');
            this.changeTimeout = setTimeout(() => {
                this.log.debug("Blinds asked to stop");
