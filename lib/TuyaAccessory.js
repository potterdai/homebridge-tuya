...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        if (!(props.id && props.key && props.ip) && !props.fake) return this.log.info('[Tuya] Insufficient details to initialize:', props);
        ...
        if (this.context.version >= 3.2) {
            ...
            //this.log.info(`[Tuya] Changing ping gap for ${this.context.name} to ${this.context.pingGap}s`);
        }
    ...
    _connect() {
        ...
        (this._socket.reconnect = () => {
            //this.log.info(`[Tuya DEBUG] reconnect called for ${this.context.name}`);
        ...
        this._socket.on('error', err => {
            this.connected = false;
            this.log.info(`[Tuya] Socket had a problem and will reconnect to ${this.context.name} (${err && err.code || err})`);

            if (err && (err.code === 'ECONNRESET' || err.code === 'EPIPE') && this._connectionAttempts < 10) {
                this.log.info(`[Tuya DEBUG] Reconnecting with connection attempts =  ${this._connectionAttempts}`);
            ...
            if (err) {
                if (err.code === 'ENOBUFS') {
                    ...
                    this.log.info('[Tuya] Slowing down retry attempts; if you see this happening often, it could mean some sort of incompatibility.');
                    delay = 60000;
            ...
            if (!this._socket._errorReconnect) {
                this.log.info(`[Tuya DEBUG] after error setting _connect in ${delay}ms`);
                this._socket._errorReconnect = setTimeout(() => {
                    this.log.info(`[Tuya DEBUG] executing _connect after ${delay}ms delay`);
                    process.nextTick(this._connect.bind(this));
                }, delay);
            }
        });
    ...
    _incrementAttemptCounter() {
        ...
        setTimeout(() => {
            this.log.info(`[Tuya DEBUG] decrementing this._connectionAttempts, currently ${this._connectionAttempts}`);
            this._connectionAttempts--;
        }, 10000);
    }
        ...
        switch (cmd) {
            ...
            case 8:
                ...
                try {
                    ...
                    data = decryptedMsg;
                    this.log.info(`[Tuya] Odd message from ${this.context.name} with command ${cmd}:`, data);
                    this.log.info(`[Tuya] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
                    break;
                }
            ...
            case 10:
                if (data) {
                    if (data === 'json obj data unvalid') {
                        this.log.info(`[Tuya] ${this.context.name} (${this.context.version}) didn't respond with its current state.`);
                        this.emit('change', {}, this.state);
                        break;
                    }
                    ...
                    try {
                        ...
                        this.log.info(`[Tuya] Malformed update from ${this.context.name} with command ${cmd}:`, data);
                        this.log.info(`[Tuya] Raw update from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
                        break;
                    }
    ...
    _msgHandler_3_3(task, callback) {
        ...
        try {
            ...
            this.log.info(`[Tuya] Odd message from ${this.context.name} with command ${cmd}:`, decryptedMsg);
            this.log.info(`[Tuya] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
            return callback();
        }
        ...
        switch (cmd) {
            ...
            case 10:
                if (data) {
                    if (data.dps) {
                        //this.log.info(`[Tuya] Heard back from ${this.context.name} with command ${cmd}`);
                        this._change(data.dps);
                    } else {
                        this.log.info(`[Tuya] Malformed message from ${this.context.name} with command ${cmd}:`, decryptedMsg);
                        this.log.info(`[Tuya] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
                    }
                }
                break;
    ...
    update(o) {
        ...
        let result = false;
        if (hasDataPoint) {
            //this.log.info("[Tuya] Sending", this.context.name, JSON.stringify(dps));
            result = this._send({
                data: {
            ...
            if (this.context.sendEmptyUpdate) {
                //this.log.info("[Tuya] Sending", this.context.name, 'empty signature');
                this._send({cmd: 7});
            }
        } else {
            //this.log.info(`[Tuya] Sending first query to ${this.context.name} (${this.context.version})`);
            result = this._send({
                data: {
